<template>
  <div class="detect-page" style="max-width:1200px; margin:0 auto">
    <!-- Upload area (manual mode) -->
    <el-card v-if="!store.liveMode" shadow="never" style="margin-bottom:20px">
      <el-upload
        ref="uploadRef"
        drag
        :auto-upload="false"
    ...
    <template v-if="store.liveMode && store.liveResults.length">
      <StatsCards
      ...
      <ResultTable
        ...
        :style="i === 0 ? 'margin-top:16px' : 'margin-top:12px;opacity:0.55'"
      />
    </template>
    ...
    <template v-if="!store.liveMode">
      ...
      <StatsCards v-if="result" :cards="statsCards" style="margin-top:20px" />
...
const liveStatsCards = computed(() => {
  ...
  const latest = store.liveResults[0]
