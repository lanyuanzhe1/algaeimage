...
> 生成日期: 2026-06-15 | 更新: 2026-06-18 | 分支: `v2-platform`
 ...
 **YOLO 输出文件夹** ：`code/algae_guardian/data/yolo_results/training/`
 ...
 **训练配置** ：YOLOv8s, imgsz=640, batch=32, AdamW, epochs=300, patience=50

```
code/algae_guardian/data/yolo_results/training/
├── args.yaml      # 训练参数
├── results.csv    # 283 epochs 完整指标
├── results.png    # 训练曲线图
├── train.log      # 完整日志
└── weights/
    └── best.pt    # epoch 233 最佳权重 (~22MB)
```

### 全部 FMPD 训练排名

| 排名        | 实验                                   | 模型    | mAP50           | mAP50-95 | 说明                 |
| ----------- | -------------------------------------- | ------- | --------------- | -------- | -------------------- |
| **1** | `yolo_results/training/`             | v8s     | **73.9%** | 47.2%    | train=val 评估，虚高 |
| 2           | `Polar_sim_0520/.../v8l_hsv/`        | v8l HSV | 61.1%           | 32.3%    | HSV 偏振             |
| 3           | `fmpd_rdn_output/.../v8l_upgrade/`   | v8l     | 42.9%           | 19.7%    | 80/20 真实划分       |
| 4           | `Polar_sim_0522/.../v8l_hsv_stable/` | v8l HSV | 37.9%           | —       | 已废弃               |
...
| 4 | `code/algae_guardian/`        | ~90,330  | tif+jpg+png               | **全管线**: TIFF→偏振→RDN→YOLO |
...
| `data/rdn_output_v2/yolo_results/` | 260  | jpg  | **YOLO 检测标注图** (`*_detect.jpg`) |

#### 3.4.3b RDN 灰度输出版（`data/rdn_output_v2_灰度版本/`）

> RDN 输出灰度版本，全部通道为单通道灰度 PNG，适合论文/竞赛文书配图。

| 子目录            | 内容                       |
| ----------------- | -------------------------- |
| `yolo_results/` | YOLO 灰度检测结果 (176 张) |

#### 3.4.4 YOLO 检测结果图

| 路径                                                           | 数量 | 内容                               | 对应训练             |
| -------------------------------------------------------------- | ---- | ---------------------------------- | -------------------- |
| `data/yolo_results/predictions/`                             | 293  | YOLO 检测标注图 (RGB)              | v8s baseline (73.9%) |
| `data/rdn_output_v2/yolo_results/`                           | 260  | YOLO 检测标注图 (`*_detect.jpg`) | RDN v2 管线          |
| `data/rdn_output_v2_灰度版本/yolo_results/`                  | 176  | YOLO 灰度检测结果                  | RDN 灰度版           |
| `data/fmpd_rdn_output/yolo_results/v8l_upgrade/val_predict/` | 40   | v8l 验证集预测标注图               | v8l upgrade (42.9%)  |

#### 3.4.5 YOLO 训练评估指标

| 路径                                                  | 关键文件                                                                                 | 对应训练                                           |
| ----------------------------------------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------- |
| `data/yolo_results/training/`                       | weights/best.pt, results.csv, results.png, train.log                                     | v8s baseline (mAP50 73.9%, train=val)              |
| `data/fmpd_rdn_output/yolo_results/v8l_upgrade/`    | confusion_matrix.png, BoxF1_curve.png, BoxPR_curve.png, results.png, val_predict/ (40张) | v8l upgrade (mAP50 42.9%, 80/20)                   |
| `data/lifewatch_yolo_results/lifewatch_v2_v8s_320/` | confusion_matrix(95×95), BoxF1/PR/P/R_curve.png, val_batch*_pred.jpg                    | **LifeWatch 95类 84.7%** (云GPU RTX 5060 Ti) |

#### 3.4.6 LifeWatch 95 类 YOLOv8s 训练输出（★ 最高指标）

> **来源**: 云 GPU 服务器 RTX 5060 Ti, 2026-05-22
> **路径**: `code/algae_guardian/data/lifewatch_yolo_results/lifewatch_v2_v8s_320/`
> **存储**: 48.6 MB

| 文件                                | 大小         | 说明                   |
| ----------------------------------- | ------------ | ---------------------- |
| `weights/best.pt`                 | 21.5 MB      | 最佳权重 (mAP50=84.7%) |
| `weights/last.pt`                 | 21.5 MB      | 最后一轮权重           |
| `results.csv`                     | 8 KB         | 训练全过程指标         |
| `results.png`                     | 292 KB       | 训练曲线总览           |
| `args.yaml`                       | 4 KB         | 训练参数               |
| `confusion_matrix.png`            | 704 KB       | 混淆矩阵 (95×95)      |
| `confusion_matrix_normalized.png` | 696 KB       | 归一化混淆矩阵         |
| `BoxF1_curve.png`                 | 868 KB       | F1-置信度曲线          |
| `BoxPR_curve.png`                 | 304 KB       | Precision-Recall 曲线  |
| `BoxP_curve.png`                  | 800 KB       | Precision-置信度曲线   |
| `BoxR_curve.png`                  | 628 KB       | Recall-置信度曲线      |
| `labels.jpg`                      | 136 KB       | 标签分布图             |
| `train_batch0~2.jpg`              | ~184 KB each | 训练批次样本 (3张)     |
| `val_batch0~2_labels.jpg`         | ~131 KB each | 验证集标签 (3张)       |
| `val_batch0~2_pred.jpg`           | ~132 KB each | 验证集预测 (3张)       |

> **注意**: LifeWatch 每张图只有一个物体 (全图 bbox)，84.7% mAP50 实质等价于 95 类分类准确率。
...
| `val/images/`   | 59 验证集图像                                                                     |
| `v8l_hsv/`      | YOLOv8l HSV 训练诊断图 (confusion matrix, F1/PR curves, results.png, mAP50 61.1%) |
