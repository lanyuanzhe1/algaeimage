...
FMPD 数据集提供 293 张图共 **3162 个人工标注框**（COCO JSON 与 YOLO label 格式完全一致），作为评估标准。

**整体对比：**

| 指标 | HSV 法 | 结构张量法 | 标注真值 |
|------|--------|-----------|----------|
| 检出框总数 | 3498 | 1919 | 3162 |
| Precision | **0.726** | 0.663 | — |
| Recall | **0.803** | 0.403 | — |
| **Overall F1** | **0.763** | **0.501** | — |

HSV 检出略多于标注（+10.6%），结构张量法严重漏检（仅覆盖 61% 标注量）。

**逐类与标注对比：**

| 类别 | 标注数 | HSV F1 | Struct F1 | HSV vs Struct |
|------|--------|--------|-----------|---------------|
| Other-phytoplankton | 794 | **0.813** | 0.543 | HSV Recall 0.84 vs 0.45 |
| Non-phytoplankton | 1723 | **0.696** | 0.348 | HSV Recall 0.74 vs 0.24 |
| Woronichinia | 233 | **0.952** | 0.892 | 均近天花板 |
| Spiroides | 58 | **0.866** | 0.682 | 小样本，HSV 优势明显 |
| Dinobryon | 354 | **0.845** | 0.640 | HSV 检出更全 |

**关键发现：**

1. **Non-phytoplankton（最大类，1723 框/54.5%）**：结构张量法召回率仅 0.24——76% 的碎屑/非藻类被漏掉。HSV 法召回 0.74，改善 3 倍。根因：结构张量法给碎屑边缘也加了偏振信号，使非藻类和藻类特征混淆；HSV 法低饱和度碎屑天然弱偏振，区分度更好。

2. **Woronichinia**：两类方法均 >0.89。该藻形态特征显著（球形群体），无论哪种增强方式 YOLO 都能稳定识别。

3. **HSV 法精度的反直觉提升**：尽管 HSV 多检出 1579 框（+82%），Precision 反而更高（0.726 vs 0.663）。说明 HSV 不是靠"撒大网"提升召回——它同时提高了命中率。
