"""Prepare train.h5 and eval.h5 from .mat file pairs.

Converts noise/truth .mat pairs into HDF5 format for RDN training.
"""
import argparse
import glob
import os
                ...
                for x in range(0, h - patch_size + 1, stride):
                    for y in range(0, w - patch_size + 1, stride):
                        lr_group.create_dataset(
                            str(patch_idx),
                            data=lr[x:x + patch_size, y:y + patch_size],
                        )
                        hr_group.create_dataset(
                            str(patch_idx),
                            data=hr[x:x + patch_size, y:y + patch_size],
                        )
                        patch_idx += 1
