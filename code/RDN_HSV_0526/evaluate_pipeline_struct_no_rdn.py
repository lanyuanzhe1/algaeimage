"""Experiment 3-Variant: Structure Tensor Polarization — No RDN.

Pipeline: RGB → Structure Tensor polarization sim → I_enh v2 → YOLOv8l → eval

This is the structure-tensor counterpart to evaluate_pipeline_no_rdn.py (HSV).
Both skip RDN entirely. Comparing the two isolates the polarization simulation
method as the single independent variable:

    Experiment 2:  HSV polarization + RDN + YOLOv8l          → mAP50 0.330
    Experiment 3:  HSV polarization (no RDN) + YOLOv8l       → mAP50 0.354
    Experiment 3b: Structure Tensor (no RDN) + YOLOv8l       → [THIS SCRIPT]

Structure tensor method: computes gradient orientation/anisotropy from the
grayscale image, then maps to polarization angles via Malus law. Unlike HSV
(color→polarization), this captures actual structural features (cell walls,
boundaries) that physically correlate with birefringence.

Compared scripts:
  - evaluate_pipeline.py              : Structure Tensor + RDN + YOLO (baseline)
  - evaluate_pipeline_no_rdn.py       : HSV (no RDN) + YOLO       (Experiment 3)
  - evaluate_pipeline_struct_no_rdn.py: Structure Tensor (no RDN) + YOLO (this file)
"""
...
sys.path.insert(0, str(BASE.parent / "Polar_sim_0520" / "image_processing"))
...
# I_enh v2 parameters (same across all experiments)
ALPHA = 0.6
BETA = 0.25
GAMMA = 0.35
CONF_THRESHOLD = 0.25
...
# ── I_enh v2 (shared with evaluate_pipeline_no_rdn.py) ────────────
def i_enh_v2_direct(pol_channels):
    """Apply I_enh v2 on polarization channels (no RDN pre-processing).

    Works identically for both HSV and Structure Tensor channels.
    The formula uses DoLP to suppress scattering (γ term) and AoP-modulated
    DoLP to enhance polarization-specific features (β term).

    Args:
        pol_channels: dict with I0, I45, I90, I135 (uint8 H×W)

    Returns:
        (H, W) uint8 enhanced grayscale
    """
...
# ── Metrics (shared across all evaluate_*.py scripts) ─────────────
def box_iou(a, b):
...
# ── Main Pipeline ──────────────────────────────────────────────────
def main():
    yolo = YOLO(str(YOLO_WEIGHTS_STRUCT))
    yolo.to(DEVICE)
    print(f"YOLO loaded: {YOLO_WEIGHTS_STRUCT}")
    ...
    for img_path in tqdm(valid_images, desc="Pipeline (Struct, no RDN)"):
        stem = img_path.stem
        ...
        # ── Pipeline (3 steps: Struct Tensor → I_enh → YOLO) ─
        t0 = time.time()

        rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)

        # Step 1: Structure Tensor polarization simulation
        #   Computes gradient orientation via structure tensor,
        #   maps to I0/I45/I90/I135 via Malus law.
        #   NO noise added — we want clean output like Experiment 3.
        pol = simulate_polarization_channels(
            rgb, polarization_strength=1.0, sigma=2.0,
            add_shot_noise=False,
        )

        # Step 2: I_enh v2 — directly from structure tensor channels (NO RDN)
        enhanced = i_enh_v2_direct(pol)

        # Step 3: YOLOv8l detection (structure-tensor-trained weights)
    ...
    print(f"\n{'='*65}")
    print(f"  Structure Tensor Pipeline (NO RDN) — FMPD 5-Class")
    print(f"{'='*65}")
    ...
    print(f"  YOLO weights:      {YOLO_WEIGHTS_STRUCT.name}")
    print(f"\n  {'Class':<25s} | {'mAP50':>8s}")
    print(f"  {'-'*25} | {'-'*8}")
    ...
    # ── Save report ──────────────────────────────────────────
    report = {
        "experiment": "3b_structure_tensor_no_rdn",
        "pipeline": "Structure Tensor → I_enh v2 → YOLOv8l (NO RDN)",
        "mAP50": float(mAP50),
        "per_class": {
        ...
        "device": DEVICE,
        "yolo_weights": str(YOLO_WEIGHTS_STRUCT),
        "note": (
            "Structure tensor polarization (no RDN, no noise). "
            "Uses structure-tensor-trained YOLOv8l weights "
            "(trained on struct+RDN+I_enh images). "
            "Compare with evaluate_pipeline_no_rdn.py (HSV variant) "
            "and evaluate_pipeline.py (struct+RDN)."
        ),
    }
    report_path = BASE / "evaluation_report_struct_no_rdn.json"
