...
function updateFileUI() {
  ...
  var isTif = /\.tiff?$/i.test(f.name);
  if (isTif) {
    // Browsers cannot render TIFF natively — upload to server for PNG preview
    var form = new FormData();
    form.append('file', f);
    fetch('/api/v1/preview', { method: 'POST', body: form })
...
function renderResult(data) {
  ...
  if (detections.length === 0) {
    ...
    listEl.innerHTML = detections.map(function (d) {
      ...
      var className = d.class_name_zh || d.class_name || d.class || 'Unknown';
